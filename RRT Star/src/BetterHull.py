...
class BetterHull():
    def __init__(self,path, k):
        ...
        self.DP = self.path_to_DP(path, 1)
            ...
            if best_node:
                ...
                while j != self.root:
                    ...
                    if not (i == a or i == b or j == a or j == b):
                        if self.intersect(a,best_node,i,j) or self.intersect(best_node,b,i,j):
                            return False
                    i = j
                    j = j.right
...
class HullSampler():
    ...
    def __init__(self, points):
        ...
        self.tri_weights = []
        for tri_points in self.points[self.tri.simplices]:
    ...
    def sample(self):
        for i in range(100):
            ...
            x = np.random.choice(range(len(self.tri.simplices)), 1, self.tri_weights)
        
            triangle = self.points[self.tri.simplices][x][0]
